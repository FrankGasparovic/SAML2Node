 ...
 * SAML2 Node
 */
@Node.Metadata(outcomeProvider = SAML2Node.SAML2NodeOutcomeProvider.class, configClass = SAML2Node.Config.class)
public class SAML2Node extends AbstractDecisionNode
{
	 ...
	 */
	private RedirectCallback configurePostRedirectCallback(final String postMsg, final String ssoURL, final RedirectCallback redirectCallback)
	{
	 ...
	 */
	private Action.ActionBuilder processError(String headerMessage, String debugMessage, Object... messageParameters) throws NodeProcessException
	{
	...
	private Action.ActionBuilder handleRedirectError(HttpServletRequest request) throws NodeProcessException
	{
		...
		{
			return processError(errorMessage, "SAML2 :: handleReturnFromRedirect() : " + "error forwarded from saml2AuthAssertionConsumer.jsp.  Error code - {}. " + "Error message - {}", String.valueOf(errorCode), errorMessage);
		}
		else if (StringUtils.isNotEmpty(errorCode))
		{
			return processError(bundle.getString(errorCode), "SAML2 :: handleReturnFromRedirect() : " + "error forwarded from saml2AuthAssertionConsumer.jsp.  Error code - {}. " + "Error message - {}", errorCode, errorMessage);
		}
		else
		{
			return processError(bundle.getString("samlVerify"), "SAML2 :: handleReturnFromRedirect() : " + "error forwarded from saml2AuthAssertionConsumer.jsp.  Error code - {}. " + "Error message - {}", errorMessage);
		}
	}
	 ...
	 * from the assertion by building them into appropriate strings and asking
	 * the auth service to migrate them into session properties once
	 * authentication is completed.
	 */
	private Map<String, Set<String>> linkAttributeValues(SPSSOConfigElement spssoConfig, Set<PrivateKey> decryptionKeys, Assertion assertion, String userName, boolean needAssertionEncrypted) throws SAML2Exception, NodeProcessException
	{
		final List<com.sun.identity.saml2.assertion.Attribute> attrs = SPACSUtils.getAttrs(assertion, SPACSUtils.getNeedAttributeEncrypted(needAssertionEncrypted, spssoConfig), decryptionKeys);
		...
		{
			logger.error("SAML Attribute Map is empty for SP: "+ spEntityID);
		...
		for (String name : attrMap.keySet())
		{
			...
			{
				// | is defined as the property value delimiter, cf
				// FMSessionProvider#setProperty
	...
	/**
	 * "Inspired" by the OAuth2 module. We use this cookie to remind us exactly
	 * where we are when returning from a remote server as we currently cannot
	 * trust the RedirectCallback's authentication framework equiv.
	 */
	private void setCookiesForRedirects(final HttpServletRequest request, final HttpServletResponse response)
	{
		...
		{
			CookieUtils.addCookieToResponse(response, CookieUtils.newCookie("authenticationStep", originalUrl.toString(), "/", domain));
		}
	}
	...
	private void removeCookiesForRedirects(final HttpServletRequest request, final HttpServletResponse response)
	{
		...
		for (String domain : domains)
		{
			CookieUtils.addCookieToResponse(response, CookieUtils.newCookie("authenticationStep", "", 0, "/", domain));
		}
	}
